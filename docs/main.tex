...
These interactions are visualised in the use case diagram below (Figure \ref{fig:use_case}), while \tablerefer{tab:use_case_actors} provides a terse overview of the role of each actor in each use case.
...
\begin{itemize}
    \item \textbf{IngestData: }The system receives a continuous stream of JSON packets from the aircraft sensors. This process includes the mandatory validation of packet syntax and structure.
...
\begin{itemize}
    ...
    \item \textbf{ThreadSafeBuffer}: collects the complete batches accumulated in the BatchAccumulator and appends them in a queue structure;
...
A sequence diagram shows how objects interact with each other over time in a system. It helps visualise the order of messages and operations between objects.
...
\begin{itemize}
    ...
    \item \textbf{RuleEngine} needs to maintain the state of previous measurements of sensors in order to check the validity of \textit{STATEFUL} rules. To do so, it is required a structure that acts as a memory and maps each \textit{sensor\_id} to the past measurements. This idea was later formalized into the MeasDatabase component.
