...
constexpr auto kPollInterval = std::chrono::milliseconds(500); // interval of polling in the ./collector_output
constexpr auto kIdleTimeout = std::chrono::seconds(5); // output of maximum idle time we can handle.
} // namespace
...
void AstraLog::readInput(const std::string &filename) {
    ...
    if (std::filesystem::is_directory(inputPath)) {
        ...
        for (const auto &entry : std::filesystem::directory_iterator(inputPath)) {
            // here we could handle the case in which we take the data from csv_input
 ...
 *  Both threads run a while loop. One waits for the batches to be accumulated on the buffer,
 *  the other one checks periodically the output directory.
 *  
 *  In the end the main thread checks wheter both the queue and the directory are empty.
 *  If they are for a certain interval it stops the execution of loops and joins threads.
 */
void AstraLog::run(const std::string &inputPath) {
    simdjson::ondemand::parser parser;
    m_evaluator->setRulesList(*m_loader, parser); // first of all load rules inside the RuleEngine
    ...
    std::atomic<bool> stopRequested{false}; // concurrent operations on std::atomic are "well-defined"
    // which basically means they do not incur in race conditons.

    // run in one thread the run loop of the RuleEngine
    std::thread evaluatorThread([this]() {
        m_evaluator->run();
    });
        ...
        while (!stopRequested.load()) {
            ...
            for (const auto &entry : std::filesystem::directory_iterator(inputDir)) {
                if (entry.path().extension() != ".txt") {
                    ...
                    // since parseTelemetry method throws some exceptions we need to handle them
                    try {
                        m_ingestor->parseTelemetry(localParser, entry.path().string());
                        std::filesystem::remove(entry.path()); 
                        // APPROACH: erase files that have been parsed --> good for storage, not so good
                        // for availability and resilience of the data. Should we consider to store them
                        // in another folder?
    ...
    while (true) {
        ...
        // if both the collector directory and the queue are empty check the system idle state
        if (dirEmpty && queueEmpty) {
            if (std::chrono::steady_clock::now() - idleStart >= kIdleTimeout) {
                stopRequested.store(true); // set to true so that the ingestor is not collecting data anymore
        ...
        std::this_thread::sleep_for(kPollInterval); // makes this thread sleep for an Interval
    }
...
int main(int argc, char** argv) {
    ...
    // basic parsing considering to pass different batch and queue size, --> I dont think we need Bucelli's library here 
    if (argc > 1) {
        inputPath = argv[1];
    }
    if (argc > 2) {
