...
class BatchAccumulator : public BatchAccumulatorInterface {
  ...
  private:
    /* =============== INTERNAL METHODS ===================*/
    // bool checkBatchSize() const;           // Return true if the new size of
    // the batch is greather than m_batcSize size_t getOverflowSize(size_t
    // addedSize) const;        // Return the number of elements that are more
    // void storeResult(std::string id, double value) const;  // Store the
    // results into the m_database void accumulate(int start);          //
    // Accumulate the batch into m_batchFile
    ...
    // TelemetryBatch                        m_batchTmp;     // better keep
    // track of the cache
    //  RuleEngineInterface&                  m_evaluator;    // interface to
    //  trigger the RuleEngine.evaluation() (will be discarded once included the
    //  queue) MeasDatabaseInterface&                m_database;     //
    //  interface to store data into the MeasDatabase class
};
