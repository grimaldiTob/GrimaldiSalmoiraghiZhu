...
class SimpleRule : public BaseRule {
  public:
     ...
     */
    SimpleRule(const std::string &rule_id, RulePriority priority,
               const std::string &sensor_id, const std::string &op,
               const double value) // no need of passing value by reference
        : BaseRule(rule_id, RuleType::SIMPLE, priority), sensor_id(sensor_id),
          op(op), value(value) {}
    ...
    std::vector<std::string> getInvolvedSensors() const {
        return {sensor_id};
    } // {} on the fly vector constructor
