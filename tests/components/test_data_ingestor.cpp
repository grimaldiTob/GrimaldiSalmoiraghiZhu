...
public:
    ...
    void storeValidData(TelemetryBatch& validBatch) override {
        ...
        for(size_t i = 0; i < validBatch.getSize(); ++i) {
            batch.emplaceBack(
                validBatch.sensors_name[i], 
                validBatch.timestamps[i], 
                validBatch.values[i], 
...
struct TempFile {
    std::string filename;
    TempFile(const std::string& name, const std::string& content) : filename(name) {
...
TEST_CASE("DataIngestor File Loading and Error Handling", "[DataIngestor][FileIO]") {
    ...
    SECTION("Throws an exception when the file does not exist") {
        ...
        REQUIRE_THROWS_AS(ingestor.parseTelemetry(parser, badPath), std::runtime_error);
    }

    SECTION("Throws an exception when the file exists but is completely empty") {
        // Create a temporary file with zero content
        TempFile emptyFile("empty_test_file.txt", "");
        REQUIRE_THROWS_AS(ingestor.parseTelemetry(parser, emptyFile.filename), std::runtime_error);
    }

    SECTION("Successfully loads and reads an existing, populated file") {
        // Create a temporary file with one valid JSON record
        std::string validData = "{\"sensor_id\": \"TEST_01\", \"timestamp\": 1680000000, \"value\": 42.0}\n";
        TempFile goodFile("good_test_file.txt", validData);

        // We require that the function executes entirely without throwing ANY exceptions
        REQUIRE_NOTHROW(ingestor.parseTelemetry(parser, goodFile.filename));
        
        // Verify that the file was not only loaded, but the data was actually parsed
...
TEST_CASE("Real File Integration Test", "[Integration][FileIO]") {
    ...
    // Require that the ingestor processes the real file without throwing any exceptions
