...
TEST_CASE("DataIngestor processes all available text files in the target directory", "[DataIngestor][BatchAccumulator]") 
{
     ...
     * then in the queue there are 2 ready batches and 40 measurements in the batch of the accumulator left
     */
    ...
    // Quick safety check
    if (!std::filesystem::exists(input_path) || std::filesystem::is_empty(input_path)) {
    ...
    // Since we have 240 total valid measurements and the batch size is 100,
    // then in queue there are two ready batches and 40 measurements in the batch of the accumulator
