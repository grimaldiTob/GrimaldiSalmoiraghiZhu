...
TEST_CASE("LogicalCorrelationRule evaluates correctly with AND logic", "[LogicalCorrelationRule]") {
    ...
    LogicalCorrelationRule corr_rule("CorrRule1", RulePriority::HIGH, "AND", conditions);
...
TEST_CASE("LogicalCorrelationRule evaluates correctly with OR logic", "[LogicalCorrelationRule]") {
    TelemetryBatch batch;
    batch.sensors_name = {"Sensor1", "Sensor2"};
    ...
    LogicalCorrelationRule corr_rule("CorrRule2", RulePriority::HIGH, "OR", conditions);
    std::unordered_map<std::string, std::optional<bool>> cache;
...
TEST_CASE("LogicalCorrelationRule works correctly when a rule is already cached", "[LogicalCorrelationRule]") {
    TelemetryBatch batch;
    ...
    LogicalCorrelationRule corr_rule("CorrRule3", RulePriority::HIGH, "AND", conditions);
    std::unordered_map<std::string, std::optional<bool>> cache;
    ...
    // Now evaluate the correlation rule, which should use the cached result for rule1
    auto corr_result = corr_rule.evaluate(batch, cache);
    REQUIRE(corr_result.has_value());
    REQUIRE(corr_result.value() == true); // The correlation rule should also evaluate to true
}

TEST_CASE("LogicalCorrelationRule returns nullopt if a condition rule returns nullopt", "[LogicalCorrelationRule]") {
    TelemetryBatch batch;
    batch.sensors_name = {"Sensor1"};
    batch.values = {10.0};

    auto rule1 = std::make_shared<SimpleRule>("Rule1", RulePriority::HIGH, "Sensor1", ">", 5.0);
    auto rule2 = std::make_shared<SimpleRule>("Rule2", RulePriority::HIGH, "Sensor2", "<", 25.0); // This will return nullopt
    std::vector<std::shared_ptr<BaseRule>> conditions = {rule1, rule2};

    LogicalCorrelationRule corr_rule("CorrRule4", RulePriority::HIGH, "AND", conditions);
    std::unordered_map<std::string, std::optional<bool>> cache;
